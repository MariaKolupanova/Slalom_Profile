...
class Graph {
public:
    Graph(std::vector<std::vector<Vertex>>& vertices, int n, int m, int road_len, int move_step, int max_deviation, double max_weight, double max_angle);
    std::vector<Coord> GetValidNeighbors(Coord v);
    std::vector<Coord> find_path_Dijkstra(Coord startPoint, Coord goalPoint, Coord CameFrom = { INF,INF });
